...
def getMultiImages( rgb_path):
   #根据jpg文件路径替换掉后缀找到TIF文件
    ret_list = None
    for i in range(1, 6):
        tmp_path = rgb_path.replace('0.JPG', str(i) + '.TIF')
        if os.path.exists(tmp_path) and tmp_path.endswith("TIF"):
            tmp_uint8 = tiff.imread(tmp_path)  # 得到的是np.array
            tmp_uint8 = np.expand_dims(tmp_uint8, 2)  # 扩展一维
            if i == 1:
                ret_list = tmp_uint8
            else:
                ret_list = np.concatenate((ret_list, tmp_uint8), axis=2)
    # 希望返回的是五通道的张量
    return ret_list
####################
